...
This is a well known no-no in Machine learning.  eg You don't take the model which has the lowest error on your training data, you use crossvalidation. [why not multiple selection on test data then?]
...
sim <- sampling(simulation, algorithm = "Fixed_param",   data=list(N_vars=100,  N_replicates=N_replicates, mu=mu, tau=tau, sigma=sigma),
			chains=1, warmup=0, iter=1000)
...
max_1 <- mean_i[,1]
max_10 <- colMax(mean_i[,1:10])
max_100 <- colMax(mean_i[,1:100])

df <- data.frame(max_1, max_10, max_100)
dfg <- df %>% gather(max_1, max_10, max_100, key="max", value="value")
ggplot(data=dfg) + geom_histogram(aes(x=value),binwidth=tau/10) + facet_grid( max ~ .)
