...
Note that the above in bayesian terms has prior $\delta(x)$, ie we assume prior has mean 0 and variance 0.
...
idxmax_10 <- matrix(c(seq(N_iter),colIdxMax(mean_i[, 1:100])), ncol=2, byrow=F)
...
for (i_iter in 1: N_iter){
  ...
  max_1[i_iter] <- mu_est[1]
  max_10[i_iter] <- max(mu_est[1:10])
  max_100[i_iter] <- max(mu_est[1:100])
}
